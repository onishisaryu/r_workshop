...
---
# Vibe coding
Generative AI will literally do everything for you these days, including writing code. <br>

It might feel like you don't need to learn the basics, but I strongly disagree.<br>

Knowing even the basics will speed up the process **a lot**. 

---
# The scope of today's workshop
Our goal today is to build the most minimal foundation on which you can stack your coding skills onto. In interest of time, we will skip:
- Legacy basics like base subsetting - easier methods exist via *packages*.
- Flow control (`if/else` statements, for loops, `purrr::map()`) and custom functions — *extremely* useful, but not required for basic analysis.
...
<div class="more-left">
<ul>
<li>Who is R statistical programming language for.</li>
--
...
- R is a specifically designed for stats (hence the popularity amongst scientists).
- R is open-source. i.e., free to use! (unlike SPSS...)
--
...
# Who is R for?
Should you be using R? Ask yourself the following:
...
# Who is R for?
These are all good reasons to consider R. But it is also possible you might not need R if:
-	Excel or SPSS is good enough – you know it and you don’t need much more.
-	Relatively simple analysis (t-test, correlation analysis) with smaller datasets.
In the end, R is another tool to use. It can make data analysis less of a hassle for some researchers, but using R doesn’t make your analysis better. 
-	R won’t magically improve your analysis, but it can make your work faster, cleaner, and reproducible — if your problems match its strengths.
---
...
--
-	The **terminal** is the system command line. Will not execute R code.

--
 - Don't worry about it unless you know bash.
...
```{r}
# to import data
data <- readr::read_csv("sample/cleanData.csv")
...
# to import data
clean_data <- data[1:4]
# write_csv(clean_data,"workshop.csv")
```
]
...
--
 - Always load libraries at the top - scattering it across your document can make it prone to bugs
 
--
 - Options (digits, scipen, etc)
...
-	What is pseudocoding? This is the process of writing out your goals in plain English before writing the code. For example:
```{r}
...
-	Keyboard shortcuts are your friend
 ...
 - Use cmd + shift + m or ctrl + shift + m for pipe operator
...
-	Use comments and headers 
 ...
 - 5 hashed ##### or #---- allows the section under it to be collapsed
...
- Camel case or dot case, not snake case.
 - I align with base R, snake case is recommended formatting. Pick one and stick with it 
- sesssionInfo()shows your R version and loaded packages
- restart session (cmd + shift + F10) – sometimes this fixes issues
